...
function saveSubmissionData(data) {
    fs.readFile('submissions.json', 'utf8', (err, fileData) => {
        ...
        let submissions = fileData ? JSON.parse(fileData) : [];
        submissions.push(data);

        fs.writeFile('submissions.json', JSON.stringify(submissions, null, 2), err => {
            if (err) console.error("Failed to save submission data:", err);
        });
    });
}
...
app.post('/submit-form', upload.fields([
    ...
    // Handle multiple skills properly
    let selectedSkills = Array.isArray(req.body.skills) ? req.body.skills.join(', ') : req.body.skills || 'None';
    let otherSkills = req.body.other_skills ? `, Other: ${req.body.other_skills}` : '';
    ...
    *Skills:* ${selectedSkills}${otherSkills}
    `;
